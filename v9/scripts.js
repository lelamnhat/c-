...
function renderProducts(products) {
  try {
    ...
    groupDiv.innerHTML = products.map(product => `
      <div class="product-item bg-white rounded-lg shadow p-4" data-id="${product.id}">
        <img src="${product.image}" alt="${product.name}" class="w-full h-48 object-cover rounded mb-2">
        <h3 class="text-lg font-semibold">${product.name}</h3>
        <p class="text-gray-600 text-sm">${product.description}</p>
        <p class="text-red-500 font-bold mt-1">${product.price.toLocaleString()} VNĐ</p>
        <button class="mt-2 bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 w-full buy-now" onclick="addToCart(${product.id})">Mua ngay</button>
      </div>
    `).join('');
      ...
      cartItem.innerHTML = `
        <div class="flex items-center">
          ...
          <div>
            ...
            <p class="text-red-500 font-bold">${item.price.toLocaleString()} VNĐ</p>
          </div>
        </div>
        <div class="flex items-center space-x-4">
          <div class="quantity-controls">
            <button onclick="updateCartQuantity(${item.id}, ${item.quantity - 1})">-</button>
            <input type="number" class="cart-quantity w-16 p-1 border rounded" value="${item.quantity}" min="1" max="${Math.min(item.stock, 10)}" data-id="${item.id}">
            <button onclick="updateCartQuantity(${item.id}, ${item.quantity + 1})">+</button>
          </div>
